...
void QImageThresholder::process(QImage & target, QImage const & source, uint8_t channel0Min, uint8_t channel0Max, uint8_t channel1Min, uint8_t channel1Max, uint8_t channel2Min, uint8_t channel2Max, int desiredColor)
{
	if (source.format() != QImage::Format_ARGB32 || channel0Min > channel0Max || channel1Min > channel1Max || channel2Min > channel2Max) {
		return; // throw?
	}
	...
	while (pixelFrom < pixelFromEnd) {
		...
		uint8_t c1{ (value >> 8) & 0xFF };
		uint8_t c2{ (value >> 0) & 0xFF };
		*pixelTo = (c0 >= channel0Min && c0 <= channel0Max && c1 >= channel1Min && c1 <= channel1Max && c2 >= channel2Min && c2 <= channel2Max) ? desiredColor : black;
		++pixelFrom;
		++pixelTo;
	}
}
